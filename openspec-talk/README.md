...
HTML-презентація для доповіді **«Дві осі SDD — специфікація і процес»** (інженерний клуб, ~50 хв).
Movement: один `slides.md` → reveal.js → статичний HTML на GitHub Pages.

> Скелет доповіді (тези, тайминг блоків) живе у vault: `Inbox/Структура доповіді — Дві осі SDD`.
> Демо (OpenSpec ~3', інтеграція ~4') ще не записані — у `slides.md` під них стоять плейсхолдери `<!-- DEMO… -->`.
> Шпаргалка з командами обох демо: [`demo.md`](demo.md).
...
npm run publish-gh-pages # збірка з base=/AI-work-place/openspec-talk/ + деплой на gh-pages
```

Опублікований URL: `https://<user>.github.io/AI-work-place/openspec-talk/`
...
бо на gh-pages сайт живе під `/AI-work-place/openspec-talk/`, і лише відносний шлях ураховує цей base.
...
> Поки що діаграм немає — `diagrams/` порожня, додаються в міру роботи над слайдами.
